...
R1, R2 = 0.5, 0.5
theta1_values = [0.1, 0.5, 2, 5, 8, 10]  # 不同的theta1值
theta2 = 0.1
rho1, rho2 = 3, 3
...
for theta1 in theta1_values:
    print(f"正在计算 theta1 = {theta1} 的情况...")
    ...
    print(f"theta1 = {theta1} 计算完成，迭代次数: {s}")
    print(f"领导者策略: N({alpha:.4f}, {Sigma_pi:.4f})")
    print(f"跟随者策略: N({gamma:.4f}, {Sigma_gamma:.4f})")

# ========================= 绘制不同theta1的领导者和跟随者的最优策略图 ================================
fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(16, 8))
colors = plt.cm.rainbow(np.linspace(0, 1, len(theta1_values)))
...
for i, theta1 in enumerate(theta1_values):
    result = results[theta1]
    pdf_leader = stats.norm.pdf(x, result['alpha'], result['Sigma_pi'])
    ax1.plot(x, pdf_leader, label=f'θ1={theta1}', color=colors[i], linewidth=2.5)

ax1.set_title('Leader Optimal Policies for Different θ1 Values', fontsize=14)
ax1.set_xlabel('Control Input', fontsize=12)
ax1.set_ylabel('Probability Density', fontsize=12)
ax1.legend(title='θ1 Values', loc='upper right', fontsize=10)
ax1.grid(True)
...
for i, theta1 in enumerate(theta1_values):
    ...
    ax2.plot(x, pdf_follower, label=f'θ1={theta1}', color=colors[i], linewidth=2.5)

ax2.set_title('Follower Optimal Policies for Different θ1 Values', fontsize=14)
ax2.set_xlabel('Control Input', fontsize=12)
ax2.set_ylabel('Probability Density', fontsize=12)
ax2.legend(title='θ1 Values', loc='upper right', fontsize=10)
ax2.grid(True)

plt.tight_layout()
plt.show()

# ========================= 绘制不同theta1的领导者和跟随者的值函数变化图 ================================
fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(16, 8))
...
for i, theta1 in enumerate(theta1_values):
    ...
    ax1.plot(iterations, result['V1_values'], label=f'θ1={theta1}', color=colors[i], linewidth=3)

ax1.set_xlabel('Iteration', fontsize=12)
ax1.set_ylabel('Value Function V1', fontsize=12)
ax1.set_title('Convergence of Leader Value Function for Different θ1 Values', fontsize=14)
ax1.legend(title='θ1 Values', loc='upper right')
ax1.grid(True)
...
for i, theta1 in enumerate(theta1_values):
    ...
    ax2.plot(iterations, result['V2_values'], label=f'θ1={theta1}', color=colors[i], linewidth=3)

ax2.set_xlabel('Iteration', fontsize=12)
ax2.set_ylabel('Value Function V2', fontsize=12)
ax2.set_title('Convergence of Follower Value Function for Different θ1 Values', fontsize=14)
ax2.legend(title='θ1 Values', loc='upper right')
ax2.grid(True)
...
# ========================= 绘制theta1对最终策略参数的影响 ================================
theta1_list = list(results.keys())
alpha_list = [results[theta1]['alpha'] for theta1 in theta1_list]
gamma_list = [results[theta1]['gamma'] for theta1 in theta1_list]
sigma_pi_list = [results[theta1]['Sigma_pi'] for theta1 in theta1_list]
sigma_gamma_list = [results[theta1]['Sigma_gamma'] for theta1 in theta1_list]

fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(16, 8))

# 均值参数图
ax1.plot(theta1_list, alpha_list, 'o-', linewidth=3, markersize=8, label='Leader Mean (α)')
ax1.plot(theta1_list, gamma_list, 's-', linewidth=3, markersize=8, label='Follower Mean (γ)')
ax1.set_xlabel('θ1 Value', fontsize=12)
ax1.set_ylabel('Policy Mean', fontsize=12)
ax1.set_title('Policy Means vs θ1', fontsize=14)
ax1.legend()
ax1.grid(True)

# 方差参数图
ax2.plot(theta1_list, sigma_pi_list, 'o-', linewidth=2, markersize=8, label='Leader Variance (Σπ)')
ax2.plot(theta1_list, sigma_gamma_list, 's-', linewidth=2, markersize=8, label='Follower Variance (Σγ)')
ax2.set_xlabel('θ1 Value', fontsize=12)
ax2.set_ylabel('Policy Variance', fontsize=12)
ax2.set_title('Policy Variances vs θ1', fontsize=14)
ax2.legend()
ax2.grid(True)
