...
for C in C_values:
    ...
    while not converged and s < max_iterations:
        ...
        W1_new = np.linalg.inv(xi1 @ xi1.T) @ xi1 @ chi1
        ...
        W2_new = np.linalg.inv(xi2 @ xi2.T) @ xi2 @ chi2
...
for i, C in enumerate(C_values):
    result = results[str(C)]
    pdf_leader = stats.norm.pdf(x, result['alpha'], result['Sigma_pi'])
    ax1.plot(x, pdf_leader, label=f'C=diag({C[0,0]},{C[1,1]})', color=colors[i], linewidth=2.5)

ax1.set_title('Leader Optimal Policies for Different C Values', fontsize=14)
ax1.set_xlabel('Control Input', fontsize=12)
ax1.set_ylabel('Probability Density', fontsize=12)
ax1.legend(title='C Values', loc='upper right', fontsize=10)
ax1.grid(True)
...
for i, C in enumerate(C_values):
    ...
    ax2.plot(x, pdf_follower, label=f'C=diag({C[0,0]},{C[1,1]})', color=colors[i], linewidth=2.5)

ax2.set_title('Follower Optimal Policies for Different C Values', fontsize=14)
ax2.set_xlabel('Control Input', fontsize=12)
ax2.set_ylabel('Probability Density', fontsize=12)
ax2.legend(title='C Values', loc='upper right', fontsize=10)
ax2.grid(True)

plt.tight_layout()
plt.show()

# ========================= 绘制不同C的领导者和跟随者的值函数变化图 ================================
fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(16, 8))
...
for i, C in enumerate(C_values):
    ...
    ax1.plot(iterations, result['V1_values'], label=f'C=diag({C[0,0]},{C[1,1]})', color=colors[i], linewidth=3)

ax1.set_xlabel('Iteration', fontsize=12)
ax1.set_ylabel('Value Function V1', fontsize=12)
ax1.set_title('Convergence of Leader Value Function for Different C Values', fontsize=14)
...
for i, C in enumerate(C_values):
    ...
    ax2.plot(iterations, result['V2_values'], label=f'C=diag({C[0,0]},{C[1,1]})', color=colors[i], linewidth=3)

ax2.set_xlabel('Iteration', fontsize=12)
ax2.set_ylabel('Value Function V2', fontsize=12)
ax2.set_title('Convergence of Follower Value Function for Different C Values', fontsize=14)
...
# ========================= 绘制C对最终策略参数的影响 ================================
c_diag_values = [f'diag({C[0,0]},{C[1,1]})' for C in C_values]
alpha_list = [results[str(C)]['alpha'] for C in C_values]
gamma_list = [results[str(C)]['gamma'] for C in C_values]
sigma_pi_list = [results[str(C)]['Sigma_pi'] for C in C_values]
sigma_gamma_list = [results[str(C)]['Sigma_gamma'] for C in C_values]

fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(16, 8))

# 均值参数图
x_pos = range(len(c_diag_values))
ax1.plot(x_pos, alpha_list, 'o-', linewidth=3, markersize=8, label='Leader Mean (α)')
ax1.plot(x_pos, gamma_list, 's-', linewidth=3, markersize=8, label='Follower Mean (γ)')
ax1.set_xlabel('C Matrix', fontsize=12)
ax1.set_ylabel('Policy Mean', fontsize=12)
ax1.set_title('Policy Means vs C Matrix', fontsize=14)
ax1.set_xticks(x_pos)
ax1.set_xticklabels(c_diag_values, rotation=45)
ax1.legend()
ax1.grid(True)

# 方差参数图
ax2.plot(x_pos, sigma_pi_list, 'o-', linewidth=3, markersize=8, label='Leader Variance (Σπ)')
ax2.plot(x_pos, sigma_gamma_list, 's-', linewidth=3, markersize=8, label='Follower Variance (Σγ)')
ax2.set_xlabel('C Matrix', fontsize=12)
ax2.set_ylabel('Policy Variance', fontsize=12)
ax2.set_title('Policy Variances vs C Matrix', fontsize=14)
ax2.set_xticks(x_pos)
ax2.set_xticklabels(c_diag_values, rotation=45)
ax2.legend()
ax2.grid(True)
