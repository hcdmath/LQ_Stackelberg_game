...
# 辅助函数定义
def phi(x):
    return np.array([x[0] ** 3, x[1] ** 3, x[0] ** 2, x[0] * x[1], x[1] ** 2, 1])
...
def compute_hessian_phi(x):
    return [
        np.array([[6 * x[0], 0], [0, 0]]),
        np.array([[0, 0], [0, 6 * x[1]]]),
        np.array([[2, 0], [0, 0]]),
        np.array([[0, 1], [1, 0]]),
        np.array([[0, 0], [0, 2]]),
        np.array([[0, 0], [0, 0]])
...
def compute_d2Vdx2(W ,x):
    hessian_phi=compute_hessian_phi(x)
    return sum(W[i] * hessian_phi[i] for i in range(6))
...
while not converged and s < max_iterations:
    # ================== 领导者更新 ==================
    # Step 2a: 计算LAM1和LAM2（使用第6个状态x[6]）
    # 使用追随者当前的值函数参数W2
    x_m = X[6].T
    d2V2_dx2 = compute_d2Vdx2(W2, x_m)
    ...
    for x in X:
        ...
        d2V1_dx2 = compute_d2Vdx2(W1, x.flatten())
    ...
    # Step 2c: 策略更新
    d2V1_new_dx2 = compute_d2Vdx2(W1_new, x_m)
    ...
    for x in X:
        ...
        d2V2_dx2 = compute_d2Vdx2(W2, x.flatten())
    ...
    # Step 3b: 策略更新
    d2V2_new_dx2 = compute_d2Vdx2(W2_new, x_m)
...
plt.plot(iterations, kl_leader, color="#d62728",linestyle='-', linewidth=2.5, label='Leader KL Divergence')
plt.plot(iterations, kl_follower, color="#000000",linestyle='--', linewidth=2.5, label='Follower KL Divergence')
plt.xlabel('Iteration', fontsize=12)
plt.ylabel('Loss Value (KL Divergence)', fontsize=12)
plt.title('KL Divergence Convergence vs Iteration', fontsize=14)
plt.legend()
plt.grid(True)
...
for i in range(6):
    plt.subplot(2, 6, i+1)
    plt.plot(W1_history[:, i], label=f'W1_{i}', color="#ff7f0e",linestyle='-', linewidth=2)
...
for i in range(6):
    plt.subplot(2, 6, i+7)
    plt.plot(W2_history[:, i], label=f'W2_{i}',color="#0414FB",linestyle='--', linewidth=2)
...
# 创建图表
plt.figure(figsize=(8, 6))
plt.plot(x, pdf_leader, label=f'Leader: N({alpha:.2f}, {Sigma_pi:.2f})', color="#d62728", linewidth=2.5, linestyle='-')
plt.plot(x, pdf_follower, label=f'Follower: N({gamma:.2f}, {Sigma_gamma:.2f})', color="#0072BD", linewidth=2.5, linestyle='--')


plt.title('Optimal Policies', fontsize=14)
plt.xlabel('Control Input', fontsize=12)
plt.ylabel('Probability Density', fontsize=12)
plt.legend(title=f'λ1={lambda1},λ2={lambda2},Θ1={theta1},Θ2={theta2}', loc='upper right', fontsize=10)
plt.grid(True)
plt.tight_layout()
plt.show()
...
# 绘制价值函数变化曲线
plt.figure(figsize=(8, 6))
plt.plot(iterations, V1_values, color="#d62728",linestyle='-', linewidth=2.5, label='Leader V1')
plt.plot(iterations, V2_values, color="#0414FB",linestyle='--', linewidth=2.5, label='Follower V2')
plt.xlabel('Iteration', fontsize=12)
